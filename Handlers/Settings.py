...
async def start_settings(user: User, message: Message):
    """
    Функция для отправки основных параметров пользователя сообщением при заходе в меню главное настроек
    :param user: объект пользователя
    :param message: объект принятого сообщения
    """
    ...
    await message.answer(answer,
                         keyboard=UserKeyboards.settings_keyboard(user))


async def settings(user: User, message: Message):
    """
    Обрабатывает всё меню настроек
    :param user: объект пользователя
    :param message: объект принятого сообщения
    """
        ...
        case 'main':
            if 'Время' in message.text:
                ...
                await message.answer('Введи время в формате HH:MM, после которого я буду присылать расписание на '
                                     'следующий день',
                                     keyboard=UserKeyboards.cancel_keyboard())
                ...
                if len(user_groups) > 2:
                    ...
                    await message.answer('Выбери группу, которую нужно удалить',
                                         keyboard=UserKeyboards.group_keyboard(groups=user.group.split(',')[1:],
            ...
            if 'Отмена' in message.text:
                ...
                await message.answer('Не могу найти такую группу. Выбери на клавиатуре или попробуй ввести заново',
                                     keyboard=UserKeyboards.group_keyboard(
                                         difflib.get_close_matches(message.text.lower(),
                                                                   groups,
                                                                   n=5)))
